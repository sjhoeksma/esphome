...
namespace esphome
{
  namespace modbus_controller
  {
    ...
    */
    ...
    void ModbusController::on_modbus_sniffer_registers(uint8_t function_code, uint16_t start_address, uint16_t number_of_registers)
    {
      ESP_LOGI(TAG, "Sniffer registered for device=%d, start_address : 0x%X(%d) - for %u registers", this->address_, start_address, start_address, number_of_registers);
      for (uint16_t idx = start_address; idx < start_address + number_of_registers; idx++)
      {
        auto reg_it = find_if(begin(register_ranges_), end(register_ranges_), [=](RegisterRange const &r)
                              { return (r.start_address == idx && r.register_type == ModbusRegisterType::SNIFFER); });
        if (reg_it == register_ranges_.end())
        {
          // Not Found add as SNIFFER register
          ESP_LOGD(TAG, "Adding sniffer register for device=%d, start_address : 0x%X", idx);
        }
        else
        {
          // Found the register,
