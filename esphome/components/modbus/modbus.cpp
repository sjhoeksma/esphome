...
namespace esphome
{
  namespace modbus
  {
    ...
    bool Modbus::parse_modbus_byte_(uint8_t byte)
    {
      ...
      {
        // Check if we as SNIFFER should change role
      ...
      for (auto *device : this->devices_)
      {
        if (device->address_ == address)
        {
          ...
          else if (this->role == ModbusRole::SNIFFER)
          {
            ...
            {
              uint16_t count = uint16_t(data[3]) | (uint16_t(data[2]) << 8);
              device->on_modbus_sniffer_registers(function_code, uint16_t(data[1]) | (uint16_t(data[0]) << 8),
                                                  count);
              this->sniffer_count[address] = count;
              this->sniffer_mode[address] = SnifferMode::CLIENT;
            }
            else if (this->sniffer_mode[address] == SnifferMode::CLIENT)
            {
              ...
              if (this->sniffer_count[address] == 0)
              {
                ...
                ESP_LOGI(TAG, "Sniffer for device=%d switched to Master", address);
              }
            }
            else
            {
      ...
      if (!found)
      {
        ESP_LOGD(TAG, "Got Modbus frame from unknown device address=%d! ", address);
      }
                      ...
                      uint8_t payload_len, const uint8_t *payload)
    {
      ...
      if (this->role == ModbusRole::SNIFFER && this->sniffer_mode[address] == SnifferMode::MASTER)
      {
